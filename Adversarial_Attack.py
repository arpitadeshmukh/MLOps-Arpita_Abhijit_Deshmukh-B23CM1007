...
os.environ["HOSTNAME"] = "sirin"
os.environ["WANDB_HOST"] = "sirin"

socket.gethostname = lambda: "sirin"

from art.estimators.classification import PyTorchClassifier
from art.attacks.evasion import FastGradientMethod, ProjectedGradientDescent, BasicIterativeMethod

WANDB_PROJECT = "MLOPS-Assignment-5-Adversarial"
...
CLIP_VALUES = (
    min((0 - m) / s for m, s in zip(MEAN, STD)),
...
def fgsm_attack(image, epsilon, data_grad):
    return torch.clamp(image + epsilon * data_grad.sign(), CLIP_VALUES[0], CLIP_VALUES[1])
...
def run_fgsm_scratch(model, test_loader, epsilon, device):
    ...
    for inputs, targets in test_loader:
        ...
        perturbed = fgsm_attack(inputs, epsilon, inputs.grad.data)
        with torch.no_grad():
...
class GaussianBlur(nn.Module):
    def __init__(self, sigma=1.0):
        ...
        g = g / g.sum()
        self.register_buffer('kernel', (g[:, None] * g[None, :]).expand(3, 1, k, k))
...
class FeatureAugment(nn.Module):
    ...
    def forward(self, x):
        return torch.cat([x, x - self.blur(x), self.sobel(x)], dim=1)
...
class FeatureDetector(nn.Module):
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            with torch.no_grad():
                new_conv.weight[:, :3, 1, 1] = backbone.conv1.weight[:, :, 3, 3]
...
def train_detector(detector, train_ds, val_ds, epochs=20, device='cuda'):
    ...
    criterion = nn.CrossEntropyLoss()
    frozen_names = {'fc', 'conv1', 'augment'}
    backbone_params = [p for n, p in detector.named_parameters() if not any(k in n for k in frozen_names)]
    new_params      = [p for n, p in detector.named_parameters() if any(k in n for k in frozen_names)]
...
def main():
    wandb.init(project=WANDB_PROJECT, name="q2_adv", settings=WANDB_SETTINGS)
    ...
    adv_sc  = fgsm_attack(inputs, 0.05, inputs.grad.data).detach().cpu()
    ...
    attack_bim = BasicIterativeMethod(
        estimator=art_classifier, eps=DETECT_EPS,
    ...
    acc_pgd = run_detector("PGD", attack_pgd, "q2_weights/detector_pgd.pth")
    acc_bim = run_detector("BIM", attack_bim, "q2_weights/detector_bim.pth")
    ...
    x_fgsm_sc  = fgsm_attack(x10d, 0.05, x10d.grad.data).detach().cpu()
    x_np10     = x10.numpy()
    x_fgsm_art = torch.tensor(fgsm_art_05.generate(x=x_np10))
    x_pgd10    = torch.tensor(attack_pgd.generate(x=x_np10))
    x_bim10    = torch.tensor(attack_bim.generate(x=x_np10))
