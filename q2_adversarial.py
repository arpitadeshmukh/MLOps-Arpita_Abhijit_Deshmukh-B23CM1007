...
class FeatureDetector(nn.Module):
    def __init__(self, pretrained=True):
        ...
        # ── FIX: the pretrained conv1 is (64, 3, 7, 7).
        # We need (64, 7, 3, 3). We can't copy the weights directly due to
        # both channel count AND kernel size mismatch.
        # Strategy: initialise the new conv with Kaiming, then copy the
        # centre pixel (index [3,3]) of the pretrained 7×7 kernel for the
        # first 3 input channels — this transfers the learned channel
        # importance while adapting to the smaller receptive field.
        ...
        if pretrained:
            with torch.no_grad():
                ...
                # centre of 7×7 kernel = index [3, 3]
                pretrained_centre = backbone.conv1.weight[:, :, 3, 3]  # (64, 3)
                # Write into the centre of our new 3×3 kernel for RGB channels
...
# ════════════════════════════════════════════════════════════════════════════
def main():
    # wandb.init(project=WANDB_PROJECT, name="Q2-Adversarial-v3")
    wandb.init(
