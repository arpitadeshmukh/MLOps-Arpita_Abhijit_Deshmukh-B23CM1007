...
def load_reviews(url, limit=2000):
    reviews = []
...
def prepare_datasets(sample_per_class=300):
    ...
    for genre, reviews in genre_reviews.items():
        reviews = random.sample(reviews, sample_per_class)
        train = reviews[: int(0.8 * sample_per_class)]
        test = reviews[int(0.8 * sample_per_class):]
