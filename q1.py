...
socket.gethostname = lambda: "rs_pgi1"

os.environ["HOSTNAME"] = "rs_pgi1"
os.environ["WANDB_HOST"] = "rs_pgi1"
# Create wandb/ HF mock logic or use env vars
...
def apply_lora(model, r=8, alpha=8, dropout=0.1):
    # Lora config for timm's ViT. The attention weights are in the 'qkv' linear layer.
    config = LoraConfig(
        r=r,
        lora_alpha=alpha,
        target_modules=["qkv"],
...
def main():
    ...
    # Enable wandb - you may need to ensure you're logged in
    # wandb.login()
    
    # EXPERIMENT 1: Full Finetune Header only (Without LoRA)
    ...
    study.optimize(objective, n_trials=9) # All 9 combinations
    
    best_params = study.best_params
    print(f"Best LoRA parameters: {best_params} with accuracy {study.best_value}")
    
    # Retrain best configuration to save it
    ...
    # Optional: Push to HuggingFace HUB (User must be logged in via `huggingface-cli login`)
    try:
        api = HfApi()
