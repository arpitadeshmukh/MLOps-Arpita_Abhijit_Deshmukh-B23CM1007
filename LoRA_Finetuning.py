...
HF_REPO = "Vit-Lora-CIFAR100-MLOPS"
...
def main():
    ...
    wandb.init(project=WANDB_PROJECT, name="Head-Only-No-LoRA", reinit=True,
               config={"experiment": "head_only", "epochs": 10}, settings=WANDB_SETTINGS)
    ...
    q1_results.append({
        "LoRA layers": "Without", "Rank": "N/A", "Alpha": "N/A", "Dropout": "N/A",
    ...
    def objective(trial):
        ...
        wandb.init(project=WANDB_PROJECT, name=exp_name, reinit=True,
                   config={"r": r, "alpha": alpha, "epochs": 10, "dropout": dropout}, settings=WANDB_SETTINGS)
        ...
        wandb.log({
            ...
            "Trainable Parameters used": trainable_params,
            "LoRA layers": "with", "Rank": r, "Alpha": alpha, "Dropout": dropout
        })
        wandb.finish()
    ...
    sampler = optuna.samplers.GridSampler({"r": [2, 4, 8], "alpha": [2, 4, 8]})
    ...
    best_lora_model = apply_lora(best_base_model, r=best_params["r"], alpha=best_params["alpha"], dropout=0.1)

    wandb.init(project=WANDB_PROJECT, name="Best_vit_lora", reinit=True, settings=WANDB_SETTINGS)
    run_experiment(best_lora_model, train_loader, val_loader, "Best_LoRA", epochs=10, device=device)
    wandb.finish()

    best_lora_model.save_pretrained("best_vit_lora_model")

    try:
        api = HfApi()
        username = api.whoami()['name']
        best_lora_model.push_to_hub(f"{username}/{HF_REPO}")
        print(f"Successfully pushed model to HuggingFace Hub: {username}/{HF_REPO}")
    except Exception as e:
        print("Could not push to HuggingFace Hub (probably not logged in). Exception:", e)

    df = pd.DataFrame(q1_results)
    df.to_csv("res.csv", index=False)
    print("\nSaved Q1 Results to res.csv for your report!")
    print(df.to_string())
